...
const TOKEN_FILE = "desktop-agent-token.bin";
...
function normalizeAppUrl(raw) {
  ...
  if (!value) {
    throw new Error(
      "Enter the application URL before syncing or uploading."
    );
  }
  ...
  if (
    ...
    throw new Error(
      "Local development URLs must use http://localhost or https://localhost."
    );
  }
...
function authHeaders(token, json = false) {
  ...
  if (!value) {
    throw new Error("The desktop agent is not signed in.");
  }
...
function safeFileName(input) {
  return (
    String(input || "captured-meeting")
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/(^-|-$)/g, "")
      .slice(0, 64) || "captured-meeting"
  );
}
...
function sendEvent(event) {
  ...
  if (win?.webContents) {
    win.webContents.send("calendar:event", event);
  }
}
...
function createWindow() {
  ...
  win.loadFile(
    path.join(__dirname, "recorder-window.html")
  );
...
app.on("second-instance", (_event, argv) => {
  const link = argv.find((arg) =>
    arg.startsWith("fathom-clone://")
  );

  if (link) {
    sendEvent(sanitizeDeepLink(link));
  }
...
app.on("open-url", (event, url) => {
  ...
  sendEvent(sanitizeDeepLink(url));
});
...
ipcMain.handle(
  "agent:login",
  async (_event, { appUrl, email, password }) => {
    ...
    const response = await fetch(
      `${baseUrl}/api/desktop-agent/login`,
      {
        method: "POST",
        headers: {
          "content-type": "application/json"
    ...
    if (!data?.token || !data?.user?.email) {
      throw new Error(
        "Login succeeded but no valid agent session was returned."
      );
    }

    saveAgentToken(data.token);
...
ipcMain.handle("agent:events", async (_event, { appUrl }) => {
  const baseUrl = normalizeAppUrl(appUrl);
  const token = getStoredAgentToken();

  const response = await fetch(
    `${baseUrl}/api/desktop-agent/events`,
    {
...
ipcMain.handle(
  ...
  async (
    ...
    if (!uploadInit.ok) {
      throw new Error(await readApiError(uploadInit));
    }
    ...
    if (
      ...
      !uploadData?.meetingId ||
      !uploadData?.objectPath
    ) {
      throw new Error(
        "The app did not return a valid recording upload URL."
      );
    }
    ...
    if (!upload.ok) {
      throw new Error(await readApiError(upload));
    }
    ...
    if (!finalize.ok) {
      throw new Error(await readApiError(finalize));
    }
...
for (const arg of process.argv) {
  if (arg.startsWith("fathom-clone://")) {
    pendingEvent = sanitizeDeepLink(arg);
  }
}
