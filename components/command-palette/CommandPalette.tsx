...
export function CommandPalette() {
  ...
  if (!open) return null;
  return <div className="command-overlay" onClick={() => setOpen(false)}><Command className="command-panel" onClick={(e) => e.stopPropagation()}><div className="toolbar" style={{padding:8}}><Search size={18}/><Command.Input value={q} onValueChange={setQ} className="input" style={{flex:1}} placeholder="Jump to a meeting, transcript moment, action item, or person" /></div><Command.List className="command-list">{results.map((r) => <Command.Item key={r.href + r.title} onSelect={() => { location.href = r.href; }} className="segment"><strong>{r.type}</strong><span>{r.title}</span></Command.Item>)}</Command.List></Command></div>;
}
